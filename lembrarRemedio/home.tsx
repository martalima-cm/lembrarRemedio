...
import {
  ...
  recordDose,
  DoseHistory,
} from "../utils/storage"; // Armazenamento
import { useFocusEffect } from "@react-navigation/native";
import {
  registerForPushNotificationsAsync,
  scheduleMedicationReminder,
} from "../utils/notifcations"; // Notificações

// Obter largura da tela
const { width } = Dimensions.get("window");

// Criar componente de círculo animado
...
function CircularProgress({
  ...
  // Renderizar círculo de progresso
  return (
...
export default function HomeScreen() {
  ...
  const loadMedications = useCallback(async () => {
    try {
      ...
      setTodaysMedications(todayMeds); // Atualizar medicamentos de hoje

      // Contar doses completadas
  ...
  useEffect(() => {
    ...
    // Adicionar ouvinte de estado do aplicativo
  ...
  // Função para registrar dose
  const handleTakeDose = async (medication: Medication) => {
    try {
      await recordDose(medication.id, true, new Date().toISOString());
      await loadMedications(); // Recarregar dados após registrar a dose
    } catch (error) {
      console.error("Erro ao registrar dose:", error);
      Alert.alert("Erro", "Falha ao registrar dose. Tente novamente.");
    }
  };

  // Verificar se a dose foi tomada
        ...
        <View style={styles.section}>
          ...
          {todaysMedications.length === 0 ? (
            ...
            todaysMedications.map((medication) => {
              const taken = isDoseTaken(medication.id); // Verificar se a dose foi tomada
              return (
